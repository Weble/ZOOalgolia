...
class RouteHelper extends AppHelper {
     ...
     * @param boolean $route If it should be run through \Joomla\CMS\Router\Route::_()
     * @param int $force_id
     *
     * @return string the route
     * @since 2.0
    ...
    protected function _findInCategoryPath($category) {
        ...
        // Ensure category tree is build and category instance is category reference from that tree
        $tree = $this->app->table->application->get($category->application_id)->getCategoryTree(true);
        $category = isset($tree[$category->id]) ? $tree[$category->id] : $category;

        foreach (array_reverse(array_keys($category->getPathway())) as $id) {
    ...
    protected function _find($type, $id) {
        if ($this->_menu_items == null) {
            ...
            $menu_items = $app->getMenu('site')->getItems('component_id', ComponentHelper::getComponent('com_zoo')->id);
            ...
            foreach ($menu_items as $menu_item) {
                switch (@$menu_item->query['view']) {
                    case 'frontpage':
                        $this->_menu_items['frontpage'][$this->app->parameter->create($menu_item->getParams())->get('application')] = $menu_item;
                        break;
                    case 'category':
                        $this->_menu_items['category'][$this->app->parameter->create($menu_item->getParams())->get('category')] = $menu_item;
                        break;
                    case 'item':
                        $this->_menu_items['item'][$this->app->parameter->create($menu_item->getParams())->get('item_id')] = $menu_item;
                        break;
                    case 'submission':
                        $this->_menu_items[(@$menu_item->query['layout'] == 'submission' ? 'submission' : 'mysubmissions')][$this->app->parameter->create($menu_item->getParams())->get('submission')] = $menu_item;
                        break;
                }
            }
        }
